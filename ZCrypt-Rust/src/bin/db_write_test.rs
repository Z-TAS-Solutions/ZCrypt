...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_url =
        env::var("DATABASE_URL").expect("DATABASE_URL missing. Please set it in .env to test DB.");
    println!("Connecting to PG Database...");

    let db = Database::init(&db_url)
        .await
        .expect("Failed to bind to postgres backend");
    ...
    match db.store_user_cryptic_record(target_custom_id, record).await {
        Ok(_) => {
            println!(
                "Successfully executed the update query for {}!",
                target_custom_id
            );
            println!(
                "If zero rows were updated, make sure the user exists and the 'cryptic_record' column is JSONB."
            );
        }
        Err(e) => {
